...
TMPDIR=/tmp/tts2
PID=$$
TMPFILE1=/tmp/tts2/tmpfile1${PID}
TMPFILE2=/tmp/tts2/tmpfile2${PID}
TTS_LOCK=$TMPDIR/lock_tts2
VOL=6
...
main() {
    ...
    do
        ...
        /usr/local/bin/wmtg_hub_tts02.py "${line[$i]}" > /dev/null 2>&1
    ...
    # sox $(ls /tmp/tts/tts_*.wav | sort -n -k1.5) /tmp/tts2/audio.wav &>/dev/null
